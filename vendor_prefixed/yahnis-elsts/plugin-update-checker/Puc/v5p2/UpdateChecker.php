...
use stdClass;
use WCPOS\Vendor\WP_Error;
if (!\class_exists(\WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\UpdateChecker::class, \false)) {
    abstract class UpdateChecker
    {
        ...
        public function __construct($metadataUrl, $directoryName, $slug = null, $checkPeriod = 12, $optionName = '')
        {
            ...
            $this->upgraderStatus = new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\UpgraderStatus();
            $this->updateState = new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\StateStore($this->optionName);
            if (did_action('init')) {
                $this->loadTextDomain();
            } else {
        ...
        public function loadTextDomain()
        {
            ...
            $locale = apply_filters('plugin_locale', is_admin() && \function_exists('WCPOS\\Vendor\\get_user_locale') ? get_user_locale() : get_locale(), $domain);
         ...
         */
        protected function fixSupportedWordpressVersion(\WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Metadata $update = null)
        {
            ...
            if (\function_exists('WCPOS\\Vendor\\get_core_updates')) {
        ...
        protected function validateApiResponse($result)
        {
            if (is_wp_error($result)) {
                /** @var WP_Error $result */
                return new \WCPOS\Vendor\WP_Error($result->get_error_code(), 'WP HTTP Error: ' . $result->get_error_message());
            }
            if (!isset($result['response']['code'])) {
                return new \WCPOS\Vendor\WP_Error('puc_no_response_code', 'wp_remote_get() returned an unexpected result.');
            }
            if ($result['response']['code'] !== 200) {
                return new \WCPOS\Vendor\WP_Error('puc_unexpected_response_code', 'HTTP response code is ' . $result['response']['code'] . ' (expected: 200)');
            }
            if (empty($result['body'])) {
                return new \WCPOS\Vendor\WP_Error('puc_empty_response', 'The metadata file appears to be empty.');
            }
            return \true;
        }
        /* -------------------------------------------------------------------
        ...
        {
            if (!\function_exists('WCPOS\\Vendor\\wp_get_installed_translations')) {
                return array();
            }
        ...
        public function fixDirectoryName($source, $remoteSource, $upgrader)
        {
            ...
            if ($source !== $correctedSource) {
                ...
                if ($this->isBadDirectoryStructure($remoteSource)) {
                    return new \WCPOS\Vendor\WP_Error('puc-incorrect-directory-structure', \sprintf('The directory structure of the update is incorrect. All files should be inside ' . 'a directory named <span class="code">%s</span>, not at the root of the ZIP archive.', \htmlentities($this->slug)));
                }
                ...
                if ($wp_filesystem->move($source, $correctedSource, \true)) {
                    ...
                    return new \WCPOS\Vendor\WP_Error('puc-rename-failed', 'Unable to rename the update to match the existing directory.');
                }
            }
            return $source;
        }
        ...
        {
            return new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\DebugBar\Extension($this);
        }
        /**
