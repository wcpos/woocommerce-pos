...
- Added: terminal settings per server-mode payment gateway under POS → Settings → Checkout → gateway → POS settings — which readers are available at the till, the default reader, and whether the till is locked to it (`default_reader`, `allowed_readers`, `lock_to_default` on the `payment_gateways` section, validated on save). The readers list comes from a new `GET wcpos/v2/settings/payment-gateways/readers?gateway_id=…&refresh=1` route answered through the `wcpos_payment_gateway_readers` filter (WCPOS Pro provides the discovery; Free alone answers 404). (wcpos/roadmap#172)
