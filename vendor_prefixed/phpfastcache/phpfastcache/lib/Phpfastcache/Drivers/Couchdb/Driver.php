...
namespace WCPOS\Vendor\Phpfastcache\Drivers\Couchdb;

use WCPOS\Vendor\Doctrine\CouchDB\CouchDBClient;
use WCPOS\Vendor\Doctrine\CouchDB\CouchDBException;
use WCPOS\Vendor\Doctrine\CouchDB\HTTP\HTTPException;
...
{
    public const COUCHDB_DEFAULT_DB_NAME = 'phpfastcache';
...
HELP;
    ...
    public function getStats() : DriverStatistic
    {
        ...
        return (new DriverStatistic())->setSize($info['sizes']['active'] ?? 0)->setRawData($info)->setData(\implode(', ', \array_keys($this->itemInstances)))->setInfo('Couchdb version ' . $this->instance->getVersion() . "\n For more information see RawData.");
    }
    /**
     * @return bool
    ...
    protected function driverConnect() : bool
    {
        ...
        $this->instance = CouchDBClient::create(['dbname' => $this->getDatabaseName(), 'url' => $url, 'timeout' => $clientConfig->getTimeout()]);
        $this->createDatabase();
        return \true;
    }
    /**
    ...
    protected function driverWrite(CacheItemInterface $item) : bool
    {
        ...
        if ($item instanceof Item) {
            try {
                $this->instance->putDocument($this->encodeDocument($this->driverPreWrap($item)), $this->getCouchDbItemKey($item), $this->getLatestDocumentRevision($this->getCouchDbItemKey($item)));
            } catch (CouchDBException $e) {
    ...
    protected function getLatestDocumentRevision($docId)
    {
        ...
        $response = $this->instance->getHttpClient()->request('HEAD', $path, null, \false);
    ...
    {
        $value[ExtendedCacheItemPoolInterface::DRIVER_DATA_WRAPPER_INDEX] = \unserialize($value[ExtendedCacheItemPoolInterface::DRIVER_DATA_WRAPPER_INDEX], ['allowed_classes' => \true]);
        $value[ExtendedCacheItemPoolInterface::DRIVER_EDATE_WRAPPER_INDEX] = new \DateTime($value[ExtendedCacheItemPoolInterface::DRIVER_EDATE_WRAPPER_INDEX]['date'], new \DateTimeZone($value[ExtendedCacheItemPoolInterface::DRIVER_EDATE_WRAPPER_INDEX]['timezone']));
        if (isset($value[ExtendedCacheItemPoolInterface::DRIVER_CDATE_WRAPPER_INDEX])) {
            $value[ExtendedCacheItemPoolInterface::DRIVER_CDATE_WRAPPER_INDEX] = new \DateTime($value[ExtendedCacheItemPoolInterface::DRIVER_CDATE_WRAPPER_INDEX]['date'], new \DateTimeZone($value[ExtendedCacheItemPoolInterface::DRIVER_CDATE_WRAPPER_INDEX]['timezone']));
        }
        if (isset($value[ExtendedCacheItemPoolInterface::DRIVER_MDATE_WRAPPER_INDEX])) {
            $value[ExtendedCacheItemPoolInterface::DRIVER_MDATE_WRAPPER_INDEX] = new \DateTime($value[ExtendedCacheItemPoolInterface::DRIVER_MDATE_WRAPPER_INDEX]['date'], new \DateTimeZone($value[ExtendedCacheItemPoolInterface::DRIVER_MDATE_WRAPPER_INDEX]['timezone']));
        }
        return $value;
    }
}
