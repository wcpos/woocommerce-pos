...
- Changed assumptions/follow-up decisions:
  - Build output is generated by `pnpm --filter @wcpos/receipt-renderer build` and ignored in git; the package `files` field still publishes `dist` once built.
  - `renderThermalPreview()` sanitizes when a browser DOM is available, but preserves usable markup in DOMParser-only runtimes to avoid breaking existing React Native/WebView preview consumers through `@wcpos/printer`.
