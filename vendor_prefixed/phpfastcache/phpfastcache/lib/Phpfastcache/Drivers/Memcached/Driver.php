...
namespace WCPOS\Vendor\Phpfastcache\Drivers\Memcached;
...
use WCPOS\Vendor\Phpfastcache\Util\MemcacheDriverCollisionDetectorTrait;
...
class Driver implements ExtendedCacheItemPoolInterface, AggregatablePoolInterface
{
    ...
    public function getStats() : DriverStatistic
    {
        ...
        return (new DriverStatistic())->setData(\implode(', ', \array_keys($this->itemInstances)))->setInfo(\sprintf("The memcache daemon v%s is up since %s.\n For more information see RawData.", $stats['version'], $date->format(\DATE_RFC2822)))->setRawData($stats)->setSize((int) $stats['bytes']);
    }
    /**
     * @return bool
     */
    protected function driverConnect() : bool
    {
        ...
        $this->instance->setOption(MemcachedSoftware::OPT_BINARY_PROTOCOL, \true);
        if ($optPrefix) {
            $this->instance->setOption(MemcachedSoftware::OPT_PREFIX_KEY, $optPrefix);
        }
        $servers = $this->getConfig()->getServers();
        if (\count($this->getConfig()->getServers()) < 1) {
            $this->getConfig()->setServers([['host' => $this->getConfig()->getHost(), 'path' => $this->getConfig()->getPath(), 'port' => $this->getConfig()->getPort(), 'saslUser' => $this->getConfig()->getSaslUser() ?: null, 'saslPassword' => $this->getConfig()->getSaslPassword() ?: null]]);
        }
