 ...
 */
namespace WCPOS\Vendor\FontLib\TrueType;

use WCPOS\Vendor\FontLib\AdobeFontMetrics;
...
class File extends BinaryStream
{
    ...
    function parseTableEntries()
    {
        ...
        $class = "WCPOS\\Vendor\\FontLib\\{$type}\\TableDirectoryEntry";
    ...
    protected function readTable($tag)
    {
        ...
        if (!self::$raw) {
            ...
            $class = "WCPOS\\Vendor\\FontLib\\Table\\Type\\{$name_canon}";
            if (!isset($this->directory[$tag]) || !@\class_exists($class)) {
                return;
            }
        } else {
            $class = "WCPOS\\Vendor\\FontLib\\Table\\Table";
        }
        /** @var Table $table */
