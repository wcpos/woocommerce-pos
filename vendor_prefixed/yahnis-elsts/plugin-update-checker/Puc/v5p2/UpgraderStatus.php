...
if (!\class_exists(\WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\UpgraderStatus::class, \false)) {
    /**
    ...
    class UpgraderStatus
    {
        ...
        private function getThingBeingUpgradedBy($upgrader)
        {
            ...
            $skin = $upgrader->skin;
            if (isset($skin->theme_info) && $skin->theme_info instanceof \WCPOS\Vendor\WP_Theme) {
                $themeDirectoryName = $skin->theme_info->get_stylesheet();
            } elseif ($skin instanceof \WCPOS\Vendor\Plugin_Upgrader_Skin) {
                if (isset($skin->plugin) && \is_string($skin->plugin) && $skin->plugin !== '') {
                    $pluginFile = $skin->plugin;
                }
            } elseif ($skin instanceof \WCPOS\Vendor\Theme_Upgrader_Skin) {
        ...
        {
            if (!\function_exists('WCPOS\\Vendor\\get_plugins')) {
