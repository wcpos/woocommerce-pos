...
use WCPOS\Vendor\Predis\Client as PredisClient;
use WCPOS\Vendor\Predis\Connection\ConnectionException as PredisConnectionException;
...
class Driver implements ExtendedCacheItemPoolInterface, AggregatablePoolInterface
{
    ...
    public function driverCheck() : bool
    {
        ...
        return \class_exists('WCPOS\\Vendor\\Predis\\Client');
    }
    /**
     * @return string
     */
...
HELP;
    ...
    public function getStats() : DriverStatistic
    {
        ...
        return (new DriverStatistic())->setData(\implode(', ', \array_keys($this->itemInstances)))->setRawData($info)->setSize((int) $size)->setInfo(\sprintf("The Redis daemon v%s is up since %s.\n For more information see RawData. \n Driver size includes the memory allocation size.", $version, $date->format(\DATE_RFC2822)));
    }
    /**
     * @return bool
    ...
    protected function driverConnect() : bool
    {
        ...
        if (!empty($this->getConfig()->getPath())) {
            $this->instance = new PredisClient(['scheme' => $this->getConfig()->getScheme(), 'persistent' => $this->getConfig()->isPersistent(), 'timeout' => $this->getConfig()->getTimeout(), 'path' => $this->getConfig()->getPath()], $options);
        } else {
