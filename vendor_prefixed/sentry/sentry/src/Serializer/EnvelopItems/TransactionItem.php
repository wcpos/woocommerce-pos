...
use WCPOS\Vendor\Sentry\Serializer\Traits\BreadcrumbSeralizerTrait;
use WCPOS\Vendor\Sentry\Tracing\Span;
use WCPOS\Vendor\Sentry\Tracing\SpanStatus;
use WCPOS\Vendor\Sentry\Tracing\TransactionMetadata;
...
class TransactionItem implements EnvelopeItemInterface
{
    ...
    {
        $result = ['trace_id' => (string) $span->getTraceId(), 'span_id' => (string) $span->getSpanId(), 'name' => $span->getDescription() ?? $span->getOp(), 'is_segment' => \false, 'start_timestamp' => $span->getStartTimestamp(), 'attributes' => \array_map(static function (Attribute $value) {
