...
- **The web app's local database no longer grows with trading volume just from searching.** The search index recorded an entry for every batch it processed, whether or not any searchable text had changed, so the stored history tracked how much you sold rather than how large your catalogue is. On installs where clean-up was failing it was never compacted, and every start-up replayed the whole history.
