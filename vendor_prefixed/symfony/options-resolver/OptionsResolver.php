 ...
 */
namespace WCPOS\Vendor\Symfony\Component\OptionsResolver;

use WCPOS\Vendor\Symfony\Component\OptionsResolver\Exception\AccessException;
use WCPOS\Vendor\Symfony\Component\OptionsResolver\Exception\InvalidArgumentException;
use WCPOS\Vendor\Symfony\Component\OptionsResolver\Exception\InvalidOptionsException;
use WCPOS\Vendor\Symfony\Component\OptionsResolver\Exception\MissingOptionsException;
use WCPOS\Vendor\Symfony\Component\OptionsResolver\Exception\NoSuchOptionException;
use WCPOS\Vendor\Symfony\Component\OptionsResolver\Exception\OptionDefinitionException;
use WCPOS\Vendor\Symfony\Component\OptionsResolver\Exception\UndefinedOptionsException;
/**
...
class OptionsResolver implements Options
{
    ...
    public function setDeprecated(string $option) : self
    {
        ...
        $args = \func_get_args();
        if (\func_num_args() < 3) {
            \WCPOS\Vendor\trigger_deprecation('symfony/options-resolver', '5.1', 'The signature of method "%s()" requires 2 new arguments: "string $package, string $version", not defining them is deprecated.', __METHOD__);
    ...
    public function offsetGet($option, bool $triggerDeprecation = \true)
    {
        ...
        if (isset($this->resolved[$option]) || \array_key_exists($option, $this->resolved)) {
            if ($triggerDeprecation && isset($this->deprecated[$option]) && (isset($this->given[$option]) || $this->calling) && \is_string($this->deprecated[$option]['message'])) {
                \WCPOS\Vendor\trigger_deprecation($this->deprecated[$option]['package'], $this->deprecated[$option]['version'], \strtr($this->deprecated[$option]['message'], ['%name%' => $option]));
            }
        ...
        if ($triggerDeprecation && isset($this->deprecated[$option]) && (isset($this->given[$option]) || $this->calling && \is_string($this->deprecated[$option]['message']))) {
            ...
            if ('' !== $message) {
                \WCPOS\Vendor\trigger_deprecation($deprecation['package'], $deprecation['version'], \strtr($message, ['%name%' => $option]));
            }
        }
