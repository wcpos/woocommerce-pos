...
class Init {
	 ...
	 * | 12 | 21 catalogue/customer/order hooks (a subset of row 10's), plus `shutdown` | `Sync\Integrity_Digest` | 10 (`shutdown` at `PHP_INT_MAX`) | unknown | Default. Shares every one of its hooks with `Sync_Journal` at the same priority, so the journal always runs first — no code found that depends on that, but nothing pins it either. Order and customer saves only MARK the digest dirty; the upsert lands on `flush_pending_digests()` at `shutdown` or before any `Digest_Index::read_digests()`. Product digests stay immediate. |
