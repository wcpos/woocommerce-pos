...
class Init {
	 ...
	 * | 10b | `pre_update_option_*`, `update_option_*`, `add_option_*` for the two `Pos_Visibility::source_options()` | `Sync\Visibility_Observer` (6 callbacks) | 10 | irrelevant | Default. Appends the journal row for a record entering or leaving the POS servable set — the transition the sequence-log stream relies on, since it drops a hidden record's update rows. Registered after `Sync_Journal` only because it writes through it. |
	...
	public function __construct() {
		...
		if ( $sync_schema_latched ) {
			...
			( new \WCPOS\WooCommercePOS\Sync\Visibility_Observer() )->register_hooks();
