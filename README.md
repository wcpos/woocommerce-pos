...
- **Templates** — a custom rewrite router serves the POS app, login, auth and checkout/receipt pages outside the normal WordPress theme. The POS UI bundle itself is loaded from `cdn.jsdelivr.net/gh/wcpos/web-bundle@<ref>/build`. One ref per lane, named after the lane: a released plugin derives `<ref>` from its own major.minor (`@1.10`, then `@1.11` once `next` becomes `main`), and the `next` lane loads `@next` — the `next` branch of `wcpos/web-bundle` _is_ its tag (dev-next sets `WCPOS_WEB_BUNDLE_REF=next`). The constant/env also serves local development.
