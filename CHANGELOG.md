...
- Added: **choose which tax-ID types your cashiers see.** WP Admin → WCPOS → Settings → General → Tax IDs now carries a "Types offered at the till" list. Untick the types your store never issues and they disappear from the "Type" dropdown on the customer and order forms — a Spanish store can leave NIF as the only choice instead of scrolling to it every time. Tax IDs already saved are never changed or hidden: a record holding a type you have since unticked keeps it, and that type stays selectable on that row. Stored as `tax_ids.enabled_types`; an empty list means "offer them all", so types added by future releases arrive switched on. Read by the POS from the new `customer_tax_id_types` store field (`Tax_Id_Settings::get_enabled_types()` / `StoreInterface::get_customer_tax_id_types()`). This is a display filter only — the REST write surface still accepts every type.
