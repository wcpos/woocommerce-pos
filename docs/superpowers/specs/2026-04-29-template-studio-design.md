...
**Status update — 2026-04-29 / PR #335 opened (`feature/receipt-renderer-pr2`):**
...
- What remains:
  - PR #335 needs review/merge before the next phase consumes the package.
  - PR 3 should start next in `monorepo-v2` after PR 2 is accepted: Template Studio tuning loop, snapshot harness, and gallery preview image workflow.
- Changed assumptions/follow-up decisions:
  ...
  - `renderThermalPreview()` sanitizes when a browser DOM is available, but preserves usable markup in DOMParser-only runtimes to avoid breaking existing React Native/WebView preview consumers through `@wcpos/printer`; PR #335 review must confirm malicious-template fixture coverage for `<script>` tags, `javascript:` URLs, inline handlers (`onerror`/`onload`), SVG with embedded JS, and HTML5 event attributes, with assertions aligned to the `wp_kses_post()` parity requirement in the Security section above.
...
2. **After PR 2:** renderer package can be imported in browser/admin contexts without native transport dependencies.
