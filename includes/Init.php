...
class Init {
	 ...
	 *
	 * Every request gets the always group; everything else is constructed only
	 * on the lanes that use it, and the order group additionally on the first
	 * order write of ANY request, so the lane classifier is an optimisation
	 * rather than a correctness gate. {@see Service_Groups} holds the membership
	 * of each group and the reasons behind it.
	 *
	 * Measured 2026-09-03: a storefront page loaded ~80 plugin files and 22
	 * objects for hooks that never fire there (see
	 * .claude/research/2026-09-03-lazy-service-construction-spec.md).
	 */
	 ...
	 * same moment on every lane. Implementation now lives in Service_Groups.
	 */
