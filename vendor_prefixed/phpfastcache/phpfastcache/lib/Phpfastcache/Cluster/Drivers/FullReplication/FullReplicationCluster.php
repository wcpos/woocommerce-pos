...
namespace WCPOS\Vendor\Phpfastcache\Cluster\Drivers\FullReplication;

use WCPOS\Vendor\Phpfastcache\Cluster\ClusterPoolAbstract;
    ...
    public function getItem($key)
    {
        ...
        if ($item && $item->isHit() && \count($poolsToResync) < \count($this->clusterPools)) {
            foreach ($poolsToResync as $poolToResync) {
                ...
                $poolItem->setEventManager($this->getEventManager())->set($item->get())->setHit($item->isHit())->setTags($item->getTags())->expiresAt($item->getExpirationDate())->setDriver($poolToResync);
