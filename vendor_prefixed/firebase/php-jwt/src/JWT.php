...
class JWT
{
    ...
    public static function decode(string $jwt, $keyOrKeyArray, \stdClass &$headers = null) : \stdClass
    {
        ...
        if (!self::verify("{$headb64}.{$bodyb64}", $sig, $key->getKeyMaterial(), $header->alg)) {
            throw new \WCPOS\Vendor\Firebase\JWT\SignatureInvalidException('Signature verification failed');
        }
        // Check the nbf if it is defined. This is the time that the
        // token can actually be used. If it's not yet that time, abort.
        if (isset($payload->nbf) && \floor($payload->nbf) > $timestamp + static::$leeway) {
            $ex = new \WCPOS\Vendor\Firebase\JWT\BeforeValidException('Cannot handle token with nbf prior to ' . \date(\DateTime::ISO8601, (int) $payload->nbf));
        ...
        if (!isset($payload->nbf) && isset($payload->iat) && \floor($payload->iat) > $timestamp + static::$leeway) {
            $ex = new \WCPOS\Vendor\Firebase\JWT\BeforeValidException('Cannot handle token with iat prior to ' . \date(\DateTime::ISO8601, (int) $payload->iat));
            $ex->setPayload($payload);
            throw $ex;
        }
        // Check if this token has expired.
        if (isset($payload->exp) && $timestamp - static::$leeway >= $payload->exp) {
            $ex = new \WCPOS\Vendor\Firebase\JWT\ExpiredException('Expired token');
            $ex->setPayload($payload);
            throw $ex;
        }
        return $payload;
     ...
     * @return Key
     */
    private static function getKey($keyOrKeyArray, ?string $kid) : \WCPOS\Vendor\Firebase\JWT\Key
    {
        if ($keyOrKeyArray instanceof \WCPOS\Vendor\Firebase\JWT\Key) {
            return $keyOrKeyArray;
        }
        if (empty($kid) && $kid !== '0') {
            throw new \UnexpectedValueException('"kid" empty, unable to lookup correct key');
        }
        if ($keyOrKeyArray instanceof \WCPOS\Vendor\Firebase\JWT\CachedKeySet) {
