...
Yes. Products are stored locally for instant search, so you can keep browsing products, building carts and completing sales during a connection drop. Offline sales queue on the device and send themselves to WooCommerce when you reconnect.
...
- **Keeps selling when the connection drops.** Sales and receipt emails queue and send themselves when you reconnect. With Pro, customer edits do too. Anything the server rejects is listed with the reason so you can fix and resend it -- nothing is silently lost.
