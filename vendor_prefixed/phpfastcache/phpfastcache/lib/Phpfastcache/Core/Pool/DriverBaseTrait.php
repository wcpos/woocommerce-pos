...
use WCPOS\Vendor\Phpfastcache\Entities\DriverIO;
use WCPOS\Vendor\Phpfastcache\Exceptions\PhpfastcacheDriverCheckException;
use WCPOS\Vendor\Phpfastcache\Exceptions\PhpfastcacheDriverConnectException;
...
trait DriverBaseTrait
{
    ...
    public function __construct(ConfigurationOption $config, $instanceId)
    {
        ...
        try {
            ...
            throw new PhpfastcacheDriverConnectException(\sprintf(self::DRIVER_CONNECT_FAILURE, $this->getDriverName(), $e->getMessage(), $e->getLine() ?: 'unknown line', $e->getFile() ?: 'unknown file'));
        }
    }
    /**
     * @return string
    ...
    public static function getConfigClass() : string
    {
        ...
        if (\class_exists($localConfigClass) && \is_a($localConfigClass, ConfigurationOption::class, \true)) {
    ...
    public function driverPreWrap(ExtendedCacheItemInterface $item) : array
    {
        $wrap = [
            ...
            // Stored but not really used, allow you to quickly identify the cache key
    ...
    {
        return \unserialize((string) $value, ['allowed_classes' => \true]);
    }
    /**
