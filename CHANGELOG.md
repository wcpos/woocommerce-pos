...
- Fixed: a terminal payment that was **authorized** for the full balance completed the order, and if the till then reloaded (or never came back) nobody captured it — the app resumes live terminal legs only on orders still open, and the ten-minute payments sweep only voided authorizations once they expired. The sweep now captures a standing authorization on an order that has already completed, through the same amount and currency verification as the capture route; a capture the provider refuses is logged and retried on the next run until the authorization expires. Orders still open are left to their till, which may yet cancel the leg. (wcpos/roadmap#169)
