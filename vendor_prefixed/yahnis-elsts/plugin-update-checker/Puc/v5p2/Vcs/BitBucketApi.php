...
use WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\OAuthSignature;
use WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Utils;
if (!\class_exists(\WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\BitBucketApi::class, \false)) {
    class BitBucketApi extends \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Api
    {
        /**
         * @var OAuthSignature
         */
        ...
        public function getBranch($branchName)
        {
            ...
            return new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Reference(array('name' => $ref, 'updated' => $branch->target->date, 'downloadUrl' => $this->getDownloadUrl($branch->name)));
        }
        /**
         * Get a specific tag.
         *
        ...
        public function getTag($tagName)
        {
            ...
            return new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Reference(array('name' => $tag->name, 'version' => \ltrim($tag->name, 'v'), 'updated' => $tag->target->date, 'downloadUrl' => $this->getDownloadUrl($tag->name)));
        }
        /**
        ...
        {
            $url = \ltrim($url, '/');
            $isSrcResource = \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Utils::startsWith($url, 'src/');
            ...
            $error = new \WCPOS\Vendor\WP_Error('puc-bitbucket-http-error', \sprintf('BitBucket API error. Base URL: "%s",  HTTP status code: %d.', $baseUrl, $code));
        ...
        public function setAuthentication($credentials)
        {
            ...
            if (!empty($credentials) && !empty($credentials['consumer_key'])) {
                $this->oauth = new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\OAuthSignature($credentials['consumer_key'], $credentials['consumer_secret']);
            } else {
                $this->oauth = null;
            }
        }
