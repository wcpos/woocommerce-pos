...
const Select = ({ options, selected, onChange }: SelectProps) => {
	return (
		<ListboxInput defaultValue={selected} onChange={onChange}>
			...
			/>
			<ListboxPopover className="mt-1 overflow-auto text-base bg-white border-0 rounded-md shadow-lg max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
				<ListboxList>
					{options.map((option) => (
