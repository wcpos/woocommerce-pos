...
describe('AutoPrintRules', () => {
	...
	it('sizes each rule select from its currently selected label', () => {
		...
		expect(screen.getByTestId('rule-scope-0')).toHaveAttribute(
			'style',
			'width: calc(11ch + 3.5rem);'
		);
		expect(screen.getByTestId('rule-printer-0')).toHaveAttribute(
			'style',
			'width: calc(7ch + 3.5rem);'
		);
		expect(screen.getByTestId('rule-template-0')).toHaveAttribute(
			'style',
