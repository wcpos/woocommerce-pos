<?php
/** Fiscal record writer tests. @package WCPOS\WooCommercePOS\Tests\Services */
...
class Test_Fiscal_Record_Writers extends WCPOS_REST_Unit_Test_Case {
	...
	private function order( bool $pos = true ): \WC_Order {
		...
		foreach ( array( '_wcpos_register' => wp_generate_uuid4(), '_wcpos_session' => wp_generate_uuid4(), '_pos_store' => 12, '_pos_user' => get_current_user_id(), '_wcpos_sale_time' => '2026-09-11T10:00:00', '_wcpos_sale_tz' => 'Europe/Madrid' ) as $key => $value ) {
	...
	public function test_existing_snapshot_is_repaired_without_new_receipt_sequence(): void {
		$order = $this->order();
		$payload = array( 'fiscal' => array( 'sequence' => 77, 'immutable_id' => 'legacy:77' ) );
	...
	public function test_online_payment_and_refund_write_no_records(): void {
		...
		$order->payment_complete();
		$refund = wc_create_refund( array( 'order_id' => $order->get_id(), 'amount' => 10, 'reason' => 'Return', 'refund_payment' => false ) );
	...
	public function test_captured_manual_void_records_before_and_after_rows(): void {
		...
		$sale = $store->find_sale( $order->get_id() );
		$row = Ledger::instance()->record( $order, array( 'id' => wp_generate_uuid4(), 'method_id' => 'pos_cash', 'amount' => '20.00' ) );
		$this->assertIsArray( $row );
		$applied = Ledger::instance()->void( $order, $row['id'], 'Changed mind' );
		$this->assertSame( 'voided', $applied['status'] );
		$records = $store->list( array( 'order_id' => $order->get_id(), 'type' => 'void' ) );
	...
	public function test_pending_and_authorized_voids_fire_action_but_write_no_records(): void {
		foreach ( array( 'pending', 'authorized' ) as $status ) {
			$order = $this->order();
			$row = array( 'id' => wp_generate_uuid4(), 'method_id' => 'pos_cash', 'amount' => '20.00', 'status' => $status, 'kind' => 'cash', 'capture_mode' => 'manual' );
			...
			$this->assertSame( array(), ( new Fiscal_Record_Store() )->list( array( 'order_id' => $order->get_id(), 'type' => 'void' ) ) );
		}
	}
	...
	public function test_cancellation_requires_pos_sale_and_is_write_once(): void {
		...
		$records = $store->list( array( 'order_id' => $order->get_id(), 'type' => 'cancellation' ) );
		$this->assertCount( 1, $records );
		$this->assertSame( $sale['id'], $records[0]['corrects_record_id'] );
		$this->assertSame( array( 'record_id' => $sale['id'], 'number' => $sale['number'], 'immutable_id' => $sale['payload']['fiscal']['immutable_id'], 'checksum' => $sale['checksum'] ), $records[0]['payload']['sale'] );
		$cart = $this->order();
	...
	public function test_admin_refunds_get_independent_numbers_and_sale_link(): void {
		...
		foreach ( array( 1, 2 ) as $number ) {
			...
			$records = $store->list( array( 'order_id' => $order->get_id(), 'type' => 'refund' ) );
