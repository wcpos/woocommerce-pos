...
use WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Update as BaseUpdate;
if (!\class_exists(\WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Theme\Update::class, \false)) {
    class Update extends \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Update
    {
        public $details_url = '';
        ...
        protected function validateMetadata($apiResponse)
        {
            ...
            foreach ($required as $key) {
                if (!isset($apiResponse->{$key}) || empty($apiResponse->{$key})) {
                    return new \WCPOS\Vendor\WP_Error('tuc-invalid-metadata', \sprintf('The theme metadata is missing the required "%s" key.', $key));
                }
            }
            return \true;
        }
