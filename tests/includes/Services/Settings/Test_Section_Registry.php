...
class Test_Section_Registry extends WP_UnitTestCase {
	...
	/**
	 * Save_settings() for an id with no registered section is an error, not a
	 * generic option write: registering a Settings Section is the only
	 * supported way to own a settings group.
	 */
	public function test_save_settings_unregistered_id_is_rejected(): void {
		$pre_save_fired = 0;
		add_filter(
		...
		$this->assertEquals( 'woocommerce_pos_settings_error', $result->get_error_code() );
		$this->assertFalse( get_option( 'woocommerce_pos_settings_thirdparty' ) );
		$this->assertEquals( 0, $pre_save_fired );
