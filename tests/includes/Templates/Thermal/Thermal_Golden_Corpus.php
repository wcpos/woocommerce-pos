...
final class Thermal_Golden_Corpus {
	...
	public static function cases(): array {
		return array(
			...
			'align-x-size'         => array(
				// Hand-derived from pre-refactor logic, not regenerated by an emitter.
				'markup'  => '
					<size width="2" height="1"><align mode="center"><text>CENTERED</text></align></size>
					<size width="2" height="1"><align mode="right"><text>RIGHT</text></align></size>
					<size height="2"><align mode="center"><text>CENTERED</text></align></size>
					<size height="2"><align mode="right"><text>RIGHT</text></align></size>
				',
				'options' => array(),
			),
			'nested-styles'       => array(
	...
	/**
	 * Case names available for a lane; existing cases cover every lane.
	 *
	 * @param string $lane The emitter lane key.
	 * @return array
	 */
	public static function case_names( string $lane ): array {
		$cases = self::cases();
		// align-x-size fixtures for epos-xml, star-markup and text are deliberately omitted.
		if ( ! in_array( $lane, array( 'escpos', 'starprnt' ), true ) ) {
			unset( $cases['align-x-size'] );
		}
	...
	public static function write_all(): array {
		...
		foreach ( self::lanes() as $lane ) {
			...
			foreach ( self::case_names( $lane ) as $case_name ) {
