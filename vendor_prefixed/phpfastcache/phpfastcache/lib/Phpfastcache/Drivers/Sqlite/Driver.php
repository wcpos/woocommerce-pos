...
namespace WCPOS\Vendor\Phpfastcache\Drivers\Sqlite;
...
use WCPOS\Vendor\Phpfastcache\Core\Pool\IO\IOHelperTrait;
use WCPOS\Vendor\Phpfastcache\Exceptions\PhpfastcacheCoreException;
use WCPOS\Vendor\Phpfastcache\Exceptions\PhpfastcacheInvalidArgumentException;
use WCPOS\Vendor\Phpfastcache\Exceptions\PhpfastcacheIOException;
...
class Driver implements ExtendedCacheItemPoolInterface, AggregatablePoolInterface
{
    ...
    {
        return \extension_loaded('pdo_sqlite') && (\is_writable($this->getSqliteDir()) || @\mkdir($this->getSqliteDir(), $this->getDefaultChmod(), \true));
    }
    /**
     * @return string
    ...
    {
        if (!\file_exists($this->getSqliteDir()) && !@\mkdir($this->getSqliteDir(), $this->getDefaultChmod(), \true)) {
            throw new PhpfastcacheIOException(\sprintf('Sqlite cannot write in "%s", aborting...', $this->getPath()));
        }
        if (!\file_exists($this->getPath())) {
            if (!\mkdir($this->getPath(), $this->getDefaultChmod(), \true)) {
                $this->fallback = \true;
            }
        }
    ...
    {
        try {
            $stm = $this->getDb($item->getEncodedKey())->prepare("SELECT * FROM `caching` WHERE `keyword`=:keyword LIMIT 1");
            $stm->execute([':keyword' => $item->getEncodedKey()]);
            $row = $stm->fetch(PDO::FETCH_ASSOC);
        } catch (PDOException $e) {
            try {
                $stm = $this->getDb($item->getEncodedKey(), \true)->prepare("SELECT * FROM `caching` WHERE `keyword`=:keyword LIMIT 1");
                $stm->execute([':keyword' => $item->getEncodedKey()]);
                $row = $stm->fetch(PDO::FETCH_ASSOC);
            } catch (PDOException $e) {
                return null;
    ...
    public function indexing($keyword)
    {
        ...
        if (isset($row['db']) && $row['db'] != '') {
            ...
            $stm->execute([':keyword' => $keyword, ':db' => $db]);
        }
        return $db;
    }
    /**
    ...
    protected function driverWrite(CacheItemInterface $item) : bool
    {
        ...
        if ($item instanceof Item) {
            try {
                $stm = $this->getDb($item->getEncodedKey())->prepare("INSERT OR REPLACE INTO `caching` (`keyword`,`object`,`exp`) values(:keyword,:object,:exp)");
                $stm->execute([':keyword' => $item->getEncodedKey(), ':object' => $this->encode($this->driverPreWrap($item)), ':exp' => $item->getExpirationDate()->getTimestamp()]);
                return \true;
            } catch (PDOException $e) {
                return \false;
            }
    ...
    protected function driverDelete(CacheItemInterface $item) : bool
    {
        ...
        if ($item instanceof Item) {
            try {
                $stm = $this->getDb($item->getEncodedKey())->prepare("DELETE FROM `caching` WHERE (`exp` <= :U) OR (`keyword`=:keyword) ");
                return $stm->execute([':keyword' => $item->getEncodedKey(), ':U' => \time()]);
            } catch (PDOException $e) {
                return \false;
            }
        } else {
