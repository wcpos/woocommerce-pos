...
class Test_Orders_Controller extends Sync_REST_Store_Test_Case {
	...
	public function test_pull_id_filter_narrows_documents_without_stalling_checkpoint(): void {
		$first   = OrderHelper::create_order();
		$second  = OrderHelper::create_order();
		$journal = new Sync_Journal();
		$journal->record_order_change( $first->get_id(), 'hook:update', false );
		$journal->record_order_change( $second->get_id(), 'hook:update', false );
		$later_sequence = $journal->head_sequence( array( 'order' ) );
		$filter = static function ( array $ids ) use ( $first ): array {
			return array_values( array_diff( $ids, array( $first->get_id() ) ) );
		};
		...
		$this->assertSame( array( $second->get_id() ), $served_ids );
