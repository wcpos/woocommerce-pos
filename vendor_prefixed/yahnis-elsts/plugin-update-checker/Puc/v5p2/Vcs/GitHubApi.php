...
use WCPOS\Vendor\Parsedown;
if (!\class_exists(\WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\GitHubApi::class, \false)) {
    class GitHubApi extends \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Api
    {
        ...
        public function getLatestRelease()
        {
            ...
            foreach ($foundReleases as $release) {
                ...
                $reference = new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Reference(array('name' => $release->tag_name, 'version' => $versionNumber, 'downloadUrl' => $release->zipball_url, 'updated' => $release->created_at, 'apiResponse' => $release));
                ...
                if ($this->releaseAssetsEnabled) {
                    ...
                    if (!empty($matchingAssets)) {
                        ...
                        if ($this->releaseAssetPreference === \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Api::REQUIRE_RELEASE_ASSETS) {
                ...
                if (!empty($release->body)) {
                    $reference->changelog = \WCPOS\Vendor\Parsedown::instance()->text($release->body);
                }
                return $reference;
            }
            return null;
        ...
        public function getLatestTag()
        {
            ...
            $tag = $versionTags[0];
            return new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Reference(array('name' => $tag->name, 'version' => \ltrim($tag->name, 'v'), 'downloadUrl' => $tag->zipball_url, 'apiResponse' => $tag));
        }
        /**
        ...
        public function getBranch($branchName)
        {
            ...
            $reference = new \WCPOS\Vendor\YahnisElsts\PluginUpdateChecker\v5p2\Vcs\Reference(array('name' => $branch->name, 'downloadUrl' => $this->buildArchiveDownloadUrl($branch->name), 'apiResponse' => $branch));
        ...
        protected function api($url, $queryParams = array())
        {
            ...
            $error = new \WCPOS\Vendor\WP_Error('puc-github-http-error', \sprintf('GitHub API error. Base URL: "%s",  HTTP status code: %d.', $baseUrl, $code));
