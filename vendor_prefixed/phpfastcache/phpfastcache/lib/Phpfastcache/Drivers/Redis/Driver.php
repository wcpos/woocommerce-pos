...
namespace WCPOS\Vendor\Phpfastcache\Drivers\Redis;
...
class Driver implements ExtendedCacheItemPoolInterface, AggregatablePoolInterface
{
    ...
    public function getStats() : DriverStatistic
    {
        ...
        return (new DriverStatistic())->setData(\implode(', ', \array_keys($this->itemInstances)))->setRawData($info)->setSize((int) $info['used_memory'])->setInfo(\sprintf("The Redis daemon v%s is up since %s.\n For more information see RawData. \n Driver size includes the memory allocation size.", $info['redis_version'], $date->format(\DATE_RFC2822)));
    }
    /**
     * @return bool
