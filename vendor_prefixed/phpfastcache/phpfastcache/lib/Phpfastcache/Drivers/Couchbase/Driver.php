...
use WCPOS\Vendor\Couchbase\Exception as CouchbaseException;
use WCPOS\Vendor\Couchbase\PasswordAuthenticator;
...
class Driver implements ExtendedCacheItemPoolInterface, AggregatablePoolInterface
{
    ...
    public function getStats() : DriverStatistic
    {
        ...
        return (new DriverStatistic())->setSize($info['basicStats']['diskUsed'])->setRawData($info)->setData(\implode(', ', \array_keys($this->itemInstances)))->setInfo('CouchBase version ' . $info['nodes'][0]['version'] . ', Uptime (in days): ' . \round($info['nodes'][0]['uptime'] / 86400, 1) . "\n For more information see RawData.");
    }
    /**
     * @return bool
    ...
    protected function driverConnect() : bool
    {
        ...
        $this->instance = new CouchbaseClient('couchbase://' . $clientConfig->getHost() . ($clientConfig->getPort() ? ":{$clientConfig->getPort()}" : ''));
    ...
    protected function driverWrite(CacheItemInterface $item) : bool
    {
        ...
        if ($item instanceof Item) {
            try {
                return (bool) $this->getBucket()->upsert($item->getEncodedKey(), $this->encodeDocument($this->driverPreWrap($item)), ['expiry' => $item->getTtl()]);
            } catch (CouchbaseException $e) {
                return \false;
            }
        }
    ...
    protected function decodeDocument(array $data) : array
    {
        ...
        $data[ExtendedCacheItemPoolInterface::DRIVER_EDATE_WRAPPER_INDEX] = \DateTime::createFromFormat(\DateTime::ATOM, $data[ExtendedCacheItemPoolInterface::DRIVER_EDATE_WRAPPER_INDEX]);
        if ($this->getConfig()->isItemDetailedDate()) {
            $data[ExtendedCacheItemPoolInterface::DRIVER_CDATE_WRAPPER_INDEX] = \DateTime::createFromFormat(\DateTime::ATOM, $data[ExtendedCacheItemPoolInterface::DRIVER_CDATE_WRAPPER_INDEX]);
            $data[ExtendedCacheItemPoolInterface::DRIVER_MDATE_WRAPPER_INDEX] = \DateTime::createFromFormat(\DateTime::ATOM, $data[ExtendedCacheItemPoolInterface::DRIVER_MDATE_WRAPPER_INDEX]);
        }
        return $data;
    }
    /********************
