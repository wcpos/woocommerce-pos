...
- **POS edits fire the same WooCommerce hooks as wp-admin, with one deliberate exception.** Creating, updating and deleting records from the till now triggers the hooks a wc/v3 write would, in the same order -- except that deleting an order from the POS restores reduced stock, while wc/v3 leaves it reduced. A customer update fired `woocommerce_update_customer` twice; it now fires once. An order paid through a real gateway is never relabelled as paid offline if a till re-sends the same payment flag.
...
- **Store Health repair fingerprints now span every record type**, not only products, so each collection can be re-synced when its payload contract changes. Barcode-setting changes are tracked automatically; extensions that filter served records can trigger a repair with `woocommerce_pos_invalidate`.
